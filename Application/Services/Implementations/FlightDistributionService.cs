...
public class FlightDistributionService : IFlightDistributionService
{
    ...
    public async Task<Result<DistributionResultDto>> AutoDistributeAsync()
    {
        ...
        var flights = await _flights.ToListAsync(
            _flights.Query()
        ...
        var existing = await _passengers.ToListAsync(
            _passengers.Query().Where(p => p.HajjYear == year));
        ...
        if (!pilgrims.Any())
            return Result.Failure<DistributionResultDto>("لا يوجد حجاج لائقون طبياً");
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure<DistributionResultDto>(ex.Message);
        }
    }
    ...
    public async Task<Result> MovePilgrimAsync(int pilgrimId, int targetDepFlightId)
    {
        ...
        var allFlights = await _flights.ToListAsync(
            _flights.Query().Where(f => f.FlightYear == year).OrderBy(f => f.FlightDate));
        ...
        var depPass = await _passengers.FirstOrDefaultAsync(
            _passengers.Query().Where(p =>
                p.PilgrimId == pilgrimId && p.HajjYear == year && depIds.Contains(p.FlightId)));

        var retPass = await _passengers.FirstOrDefaultAsync(
            _passengers.Query().Where(p =>
                p.PilgrimId == pilgrimId && p.HajjYear == year && retIds.Contains(p.FlightId)));
